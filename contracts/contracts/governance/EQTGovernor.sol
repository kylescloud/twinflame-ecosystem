...
/// @title EQTGovernor — treasury & dividend governance using EQT voting power (KYC-only holders)
contract EQTGovernor is
    Governor,
    GovernorSettings,
...
{
    constructor(IVotes eqtToken, TimelockController timelock)
        Governor("TwinFlame EQT Governor")
        GovernorSettings(7200, 50_400, 1_000 ether) // 1d delay, 7d period, 1k EQT threshold
        GovernorVotes(eqtToken)
        GovernorVotesQuorumFraction(10) // 10% quorum (smaller, KYC'd holder set)
